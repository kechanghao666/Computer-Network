...
class Member4ServiceTest(unittest.TestCase):
    ...
    def test_withdraw_money_updates_balance_and_records_flow(self):
        ...
        self.assertEqual(get_balance("zhangsan", self.db_path, record_flow=False), 4500.0)

        flows = get_flow("zhangsan", self.db_path, record_flow=False)
    ...
    def test_deposit_money_updates_balance_and_records_flow(self):
        ...
        self.assertEqual(get_balance("lisi", self.db_path, record_flow=False), 3300.0)

        flows = get_flow("lisi", self.db_path, record_flow=False)
        self.assertEqual(len(flows), 1)
    ...
    def test_invalid_amount_does_not_change_balance_or_add_flow(self):
        ...
        self.assertEqual(get_balance("zhangsan", self.db_path, record_flow=False), 5000.0)
        self.assertEqual(get_flow("zhangsan", self.db_path, record_flow=False), [])
    ...
    def test_insufficient_balance_does_not_change_balance_or_add_flow(self):
        ...
        self.assertEqual(get_balance("lisi", self.db_path, record_flow=False), 3000.0)
        self.assertEqual(get_flow("lisi", self.db_path, record_flow=False), [])

    def test_add_flow_records_query_operation(self):
        add_flow("admin", "BALANCE_QUERY", 0, 10000, self.db_path, "查询余额")

        flows = get_flow("admin", self.db_path, record_flow=False)
        self.assertEqual(len(flows), 1)
        self.assertEqual(flows[0]["type"], "BALANCE_QUERY")
        self.assertEqual(flows[0]["remark"], "查询余额")

    def test_get_balance_records_balance_query_flow(self):
        balance = get_balance("zhangsan", self.db_path)

        self.assertEqual(balance, 5000.0)
        flows = get_flow("zhangsan", self.db_path, record_flow=False)
        self.assertEqual(len(flows), 1)
        self.assertEqual(flows[0]["type"], "BALANCE_QUERY")
        self.assertEqual(flows[0]["amount"], 0.0)
        self.assertEqual(flows[0]["balance_after"], 5000.0)

    def test_get_flow_records_flow_query_operation(self):
        withdraw_money("zhangsan", 100, self.db_path)

        flows = get_flow("zhangsan", self.db_path)

        self.assertEqual([flow["type"] for flow in flows], ["WITHDRAW", "FLOW_QUERY"])
    ...
    def test_send_email_returns_false_when_sender_setup_fails(self):
        old_port = os.environ.get("ATM_SMTP_PORT")
        os.environ["ATM_SMTP_PORT"] = "bad-port"
        try:
            self.assertFalse(send_email("zhangsan@qq.com", "subject", "content"))
        finally:
            if old_port is None:
